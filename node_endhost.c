...
void dwm_rx(uint8_t* rx_buffer, int len_rx_buffer);
...
//#define RX_RESP_TO_UUS 20000
//#define RX_RESP_TO_UUS 30000
#define RX_RESP_TO_UUS 3000000
...
int node_endhost(uint8_t device_id, uint8_t adj_id)
{
    ...
    */
    ...
    if(rx_buffer[TYPE_IDX] == TYPE_CTS)
    {
      ...
      while(fread(&tx_msg[MSG_IDX], sizeof(uint8_t), buf_len-5, myFile)!=0){    // CHANGED from 4 to 5
          //Reset ack2 flag
          ack2 = FALSE;
          ...
          while(1){
            ...
            if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
            {
                ...
                if((rx_buffer[DST_IDX]==device_id) && (rx_buffer[SRC_IDX]==adj_id) && ((rx_buffer[TYPE_IDX]==TYPE_ACK_1) || (rx_buffer[TYPE_IDX]==TYPE_ACK_2)))
                {
      ...
      //dwt_writetxdata((sizeof(done_msg)+FCS_LEN)-FCS_LEN, done_msg, 0); /* Zero offset in TX buffer. */
      //dwt_writetxfctrl((sizeof(done_msg)+FCS_LEN), 0, 0); /* Zero offset in TX buffer, no ranging. */
      //dwt_starttx(DWT_START_TX_IMMEDIATE);
      //while (!(dwt_read32bitreg(SYS_STATUS_ID) & SYS_STATUS_TXFRS_BIT_MASK))
      //{ };
      //dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_TXFRS_BIT_MASK);
      ////test_run_info((unsigned char *)tx_msg);
      ...
      dwm_tx(CTS,sizeof(CTS));
        ...
        //myFile = fopen("rx_image.png","wb+");
          ...
          do{
            ...
            //      if((rx_buffer[DST_IDX]==adj_id-(device_id-adj_id)) && (rx_buffer[SRC_IDX] = adj_id) && ((rx_buffer[TYPE_IDX]==TYPE_ACK_1) || (rx_buffer[TYPE_IDX]==TYPE_ACK_2)))
            //      {
            //        Sleep(30); //Give the relay node time to switch from TX to RX
            //        /* TX ACK 2 to adj node*/
            //        //FRAME_LENGTH -> (sizeof(ack_rx)+FCS_LEN)
            //        ack_rx[TYPE_IDX] = TYPE_ACK_2;
            //        dwt_writetxdata((sizeof(ack_rx)+FCS_LEN)-FCS_LEN, ack_rx, 0); /* Zero offset in TX buffer. */
            //        dwt_writetxfctrl((sizeof(ack_rx)+FCS_LEN), 0, 0); /* Zero offset in TX buffer, no ranging. */
            //        dwt_starttx(DWT_START_TX_IMMEDIATE);
            //        while (!(dwt_read32bitreg(SYS_STATUS_ID) & SYS_STATUS_TXFRS_BIT_MASK))
            //        { };
            //        dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_TXFRS_BIT_MASK | SYS_STATUS_RXFCG_BIT_MASK);
...
void dwm_tx(uint8_t* tx_buffer, int len_tx_buffer) {
  // len_tx_buffer = sizeof(tx_buffer)
  dwt_writetxdata(len_tx_buffer, tx_buffer, 0); /* Zero offset in TX buffer. */
  dwt_writetxfctrl(len_tx_buffer+FCS_LEN, 0, 0); /* Zero offset in TX buffer, no ranging. */
