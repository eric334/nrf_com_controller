 ...
 */
int tx(unsigned int *transmit_buffer, int size)
{
    //printf("%.0d",tx_msg[0]);

    #define FRAME_LENGTH    (size+FCS_LEN)
    //printf("%.0d",sizeof(transmit_buffer));

    /* Display application name on LCD. */
    printf((unsigned char *)APP_NAME);
    ...
    while(count < 2)
    {
        ...
        dwt_writetxdata(FRAME_LENGTH-FCS_LEN, transmit_buffer, 0); /* Zero offset in TX buffer. */
        ...
        //test_run_info((unsigned char *)transmit_buffer);

        /* Execute a delay between transmissions. */
        //Sleep(TX_DELAY_MS);

        /* Increment the blink frame sequence number (modulo 256). */
        //transmit_buffer[BLINK_FRAME_SN_IDX]++;
    }
}
