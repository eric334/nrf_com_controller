...
int node_relay(uint8_t device_id)
{
    ...
    do{
      ...
      //while(fread(&rx_buffer[MSG_IDX], sizeof(uint8_t), buf_len-4, myFile)!=0){
        ...
        // RX ACK 2 from next node - COMMENT OUT FOR NODE TEST 1 -> 2
        if(notFirst){
          while(1){
            ...
            if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
            {
                ...
                if((ack_rx[DST_IDX]==device_id) && (ack_rx[SRC_IDX]==tx_id) && (ack_rx[TYPE_IDX]==TYPE_ACK_2))
                {
                  test_run_info((unsigned char *)"Received ACK 2\n");
                  dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXFCG_BIT_MASK);
                  break;
                }
                dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXFCG_BIT_MASK);
        ...
        memcpy(tx_msg,rx_buffer,sizeof(tx_msg));
        
        tx_msg[TYPE_IDX] = rx_buffer[TYPE_IDX];
        tx_msg[LEN_IDX] = buf_len-4;
        tx_msg[SRC_IDX] = device_id;
        ...
        dwt_writetxdata((sizeof(tx_msg)+FCS_LEN)-FCS_LEN, tx_msg, 0); /* Zero offset in TX buffer. */
        dwt_writetxfctrl((sizeof(tx_msg)+FCS_LEN), 0, 0); /* Zero offset in TX buffer, no ranging. */
        dwt_starttx(DWT_START_TX_IMMEDIATE);
        while (!(dwt_read32bitreg(SYS_STATUS_ID) & SYS_STATUS_TXFRS_BIT_MASK))
        { };
        dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_TXFRS_BIT_MASK);
        //test_run_info((unsigned char *)"TX msg to 3\n");
        ...
        while(1){
          ...
          {
              if (tx_msg[TYPE_IDX]==TYPE_LAST) {
                test_run_info((unsigned char *)"IMAGE TX --- DONE!");
                return 1;
              }
              // Re-TX msg to next node
               dwt_writetxdata((sizeof(tx_msg)+FCS_LEN)-FCS_LEN, tx_msg, 0); /* Zero offset in TX buffer. */
               dwt_writetxfctrl((sizeof(tx_msg)+FCS_LEN), 0, 0); /* Zero offset in TX buffer, no ranging. */
               dwt_starttx(DWT_START_TX_IMMEDIATE);
               while (!(dwt_read32bitreg(SYS_STATUS_ID) & SYS_STATUS_TXFRS_BIT_MASK))
      ...
      //// Testing node 2 -> 3
      //uint8_t done_msg[] = {device_id, tx_id, TYPE_LAST, 0};
      ///* TX DONE_MSG*/
